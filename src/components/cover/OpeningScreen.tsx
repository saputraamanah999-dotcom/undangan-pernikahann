...
import { Sparkles, Calendar, Bell, BellOff, ShieldAlert } from 'lucide-react';
import { useNotificationPermission } from '../../hooks/useNotificationPermission';
import { updateVisitorNotifStatus } from '../../hooks/useVisitorTracker';
...
export function OpeningScreen({ guestName, preweddingUrl, couple1Names, couple2Names, isJointWedding, onOpen, forceNotifAndroid, forceNotifIos }: OpeningScreenProps) {
  ...
  const shouldForceNotif = isIOS() ? forceNotifIos : isAndroid() ? forceNotifAndroid : forceNotifAndroid;

  const handleOpenClick = async () => {
    setIsOpening(true);

    // If notifications not supported at all
    ...
    if (Notification.permission === 'granted') {
      try { await requestPermission(); } catch (e) { /* token fetch non-critical */ }
      setTimeout(() => onOpen(), 400);
      return;
    }
    ...
    try {
      ...
      if (result === 'granted') {
        ...
        updateVisitorNotifStatus(guestName);
        setTimeout(() => onOpen(), 400);
      } else {
        // User blocked or dismissed
        if (shouldForceNotif) {
          setNotifState('denied');
          updateVisitorNotifStatus(guestName);
          setIsOpening(false);
        } else {
          // Not forced → allow through anyway
          updateVisitorNotifStatus(guestName);
          setTimeout(() => onOpen(), 400);
        }
      }
      ...
      updateVisitorNotifStatus(guestName);
      // Request failed (e.g. iOS Safari older version)
      if (shouldForceNotif) {
        setNotifState('unsupported');
        setIsOpening(false);
      } else {
        ...
        >
          ...
          {notifState === 'denied' && (
            <motion.div
              ...
              className="w-full max-w-[320px] mb-4 p-4 rounded-2xl bg-red-500/10 border border-red-500/30 flex flex-col items-center gap-2"
            >
              <ShieldAlert className="w-6 h-6 text-red-400" />
              <p className="text-[11px] text-red-300 font-bold text-center leading-relaxed">
                Notifikasi wajib diizinkan untuk membuka undangan.
              </p>
              <p className="text-[9px] text-white/50 text-center leading-relaxed">
                Klik ikon gembok di address bar browser, lalu ubah notifikasi menjadi <span className="text-amber-300 font-bold">Izinkan</span>. Setelah itu, klik tombol di bawah lagi.
              </p>
            </motion.div>
          )}

          {/* Notification unsupported warning (iOS) */}
          {notifState === 'unsupported' && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="w-full max-w-[320px] mb-4 p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex flex-col items-center gap-2"
            >
              <BellOff className="w-6 h-6 text-amber-400" />
              <p className="text-[11px] text-amber-200 font-bold text-center leading-relaxed">
                Browser ini tidak mendukung notifikasi.
              </p>
              <p className="text-[9px] text-white/50 text-center leading-relaxed">
                {isIOS()
                  ? "Buka undangan ini di Chrome untuk iPhone, atau hubungi admin untuk menonaktifkan paksa notifikasi iOS."
                  : "Silakan buka dengan Chrome atau Firefox."}
              </p>
            </motion.div>
          )}
          ...
          >
            {notifState !== 'pending' && shouldForceNotif ? (
              <>
                <Bell className="w-4 h-4 text-white" />
                <span>{isOpening ? 'Mengecek...' : 'Coba Lagi'}</span>
              </>
            ) : (
              <>
