...
export function OpeningScreen({ guestName, preweddingUrl, coupleNames, familyLabel, onOpen }: OpeningScreenProps) {
  ...
  const handleOpenClick = async () => {
    ...
    // If already denied, block and show message
    ...
    // Request permission — this must be in a user gesture (click handler)
    try {
      ...
      if (result === 'granted') {
        ...
        // User clicked "Block" or dismissed
        setNotifState('denied');
        setIsOpening(false);
      }
    } catch (e) {
    ...
    <div className="relative h-dvh w-full overflow-hidden flex items-center justify-center bg-[#1e0a17]">
      ...
      <div className="relative z-10 flex h-full w-full max-w-md flex-col items-center justify-between py-16 px-6 text-white text-center">
        ...
        >
              ...
              <button
                onClick={() => {
                  ...
                  if ('Notification' in window && Notification.permission === 'denied') {
                    window.open('https://support.google.com/chrome/answer/6148059', '_blank');
                  }
                }}
                className="mt-1 px-4 py-2 rounded-lg bg-red-500/20 border border-red-500/30 text-red-300 text-[10px] font-bold uppercase tracking-wider hover:bg-red-500/30 cursor-pointer active:scale-95 transition-all"
              >
          ...
          {/* Notification unsupported warning */}
          {notifState === 'unsupported' && (
            <motion.div
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="w-full max-w-[320px] mb-4 p-3 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center gap-2"
            >
              <BellOff className="w-4 h-4 text-amber-400 shrink-0" />
              <p className="text-[9px] text-amber-200/80 leading-relaxed">
                Browser ini tidak mendukung notifikasi. Silakan buka dengan Chrome atau Firefox.
              </p>
            </motion.div>
          )}
          ...
          >
            {notifState === 'denied' ? (
              <>
                ...
                <span>{isOpening ? 'Mengecek...' : 'Izinkan Notifikasi Dulu'}</span>
              </>
            ) : (
              <>
